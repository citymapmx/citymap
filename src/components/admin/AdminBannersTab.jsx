    ...
    <>
      {!banForm && <div>
        ...
        {data.banners.map(bn => {
          ...
          return <div key={bn.id} style={{ background: "#fff", borderRadius: 12, marginBottom: 10, overflow: "hidden", boxShadow: "0 2px 8px rgba(0,0,0,.05)" }}>
            {bn.img_url && <div style={{ height: 80, overflow: "hidden", background: "#F7F8F6" }}>{bn.img_url.match(/\.(mp4|webm)(\?.*)?$/i) ? <video src={bn.img_url} autoPlay muted loop playsInline style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : (bn.img_url.includes("youtube") || bn.img_url.includes("youtu.be") || bn.img_url.includes("vimeo")) ? <div style={{width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", background: "#0F1A14", color: "#fff", fontSize: 11, fontWeight: 700}}>VIDEO EXTERNO</div> : <OptimizedImage src={bn.img_url} widthRequest={800} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} />}</div>}
      ...
      {banForm && <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        <div style={{ background: "#fff", borderRadius: 14, padding: 16, boxShadow: "0 2px 8px rgba(0,0,0,.05)", display: "flex", flexDirection: "column", gap: 11 }}>
          ...
          <div><div className="text-xs" style={{ fontWeight: 700, color: "#5A6872", textTransform: "uppercase", letterSpacing: .8, marginBottom: 6 }}>Multimedia (Imagen o Video MP4)</div><Uploader accept="image/*,video/mp4,video/webm" aspect={21/9} onDone={url => setBanForm(f => ({ ...f, img_url: url }))} /><div style={{textAlign: "center", margin: "10px 0", fontSize: 12, color: "#9CA3AF"}}>O pegar link de YouTube / Vimeo:</div><input type="text" value={banForm.img_url || ""} onChange={e => setBanForm(f => ({...f, img_url: e.target.value}))} placeholder="https://youtube.com/..." style={{width: "100%", padding: "10px 12px", border: "1.5px solid #E4E8E4", borderRadius: 10, fontSize: 13, fontFamily: "inherit"}} />{banForm.img_url && (banForm.img_url.match(/\.(mp4|webm)(\?.*)?$/i) ? <video src={banForm.img_url} autoPlay muted loop playsInline style={{ width: "100%", height: 100, objectFit: "cover", borderRadius: 8, marginTop: 8 }} /> : (banForm.img_url.includes("youtube") || banForm.img_url.includes("youtu.be") || banForm.img_url.includes("vimeo")) ? <div style={{marginTop: 8, padding: 10, background: "#EAF4F0", color: "#1A7A5E", borderRadius: 8, fontSize: 13, fontWeight: 600, textAlign: "center"}}>Video vinculado correctamente</div> : <OptimizedImage src={banForm.img_url} widthRequest={800} alt="" style={{ width: "100%", height: 100, objectFit: "cover", borderRadius: 8, marginTop: 8 }} />)}</div>
        </div>
