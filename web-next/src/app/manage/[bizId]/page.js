import OwnerDashboardClient from '../../../components/manage/OwnerDashboardClient';
import { notFound } from 'next/navigation';

export const metadata = { title: 'Panel de Administración - CityMap' };
...
export default async function ManageBizPage({ params }) {
  const { bizId } = await params;
  const biz = await getBusiness(bizId);
  if (!biz) return notFound();
  
  return <OwnerDashboardClient biz={biz} />;
}
