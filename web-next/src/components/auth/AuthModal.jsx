...
export default function AuthModal({ T, dark, cities, doAuth }) {
  ...
  return (
    <div className="fixed inset-0 z-[999] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200" onClick={() => setShowAuth(false)}>
      <div className="bg-white w-full max-w-[360px] rounded-[32px] p-6 shadow-2xl relative animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
      ...
      {/* ── Botón Google ── */}
      <button className="active:scale-[0.98] transition-transform w-full" onClick={async () => {
        setAuthErr("");
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        {authMode === "register" && (
          <>
            <div style={{ position: "relative" }}>
              ...
              <input className="w-full" placeholder="Tu nombre y apellido" value={authForm.name} onChange={e => setAuthForm(f => ({ ...f, name: e.target.value }))} style={{ padding: "14px 16px 14px 42px", borderRadius: 12, border: `1.5px solid ${T.border}`, fontSize: 15, background: T.white, color: T.text, outline: "none", width: "100%" }} />
            </div>
            <select className="w-full" style={{ padding: "14px 16px", borderRadius: 12, border: `1.5px solid ${T.border}`, fontSize: 15, background: T.white, color: T.text, outline: "none", appearance: "none" }} value={authForm.city} onChange={e => setAuthForm(f => ({ ...f, city: e.target.value }))}>
        ...
        <div style={{ position: "relative" }}>
          ...
          <input className="w-full" placeholder="Correo electrónico" type="email" value={authForm.email} onChange={e => setAuthForm(f => ({ ...f, email: e.target.value }))} style={{ padding: "14px 16px 14px 42px", borderRadius: 12, border: `1.5px solid ${T.border}`, fontSize: 15, background: T.white, color: T.text, outline: "none", width: "100%" }} />
        </div>
        <div style={{ position: "relative" }}>
          <span style={{ position: "absolute", left: 14, top: "50%", transform: "translateY(-50%)", display: "flex", alignItems: "center", pointerEvents: "none" }}>
            <Icon name="lock" size={16} color={T.sub} />
          </span>
          <input className="w-full" placeholder="Contraseña" type="password" value={authForm.password} onChange={e => setAuthForm(f => ({ ...f, password: e.target.value }))} style={{ padding: "14px 16px 14px 42px", borderRadius: 12, border: `1.5px solid ${T.border}`, fontSize: 15, background: T.white, color: T.text, outline: "none", width: "100%" }} />
        </div>
      </div>

      {/* ── CTA ── */}
      <button className="active:scale-[0.98] transition-transform w-full" style={{ marginTop: 20, padding: "16px", borderRadius: 14, background: dark ? "#f1f5f9" : "#0f172a", color: dark ? "#0f172a" : "#fff", fontWeight: 800, fontSize: 15, width: "100%", border: "none", cursor: "pointer", boxShadow: "0 8px 24px rgba(0,0,0,0.18)", opacity: authLoading ? 0.7 : 1 }} onClick={doAuth} disabled={authLoading}>
